...
import type { Acabado, TipoImpactoPrecio } from '../../types/database';

interface AcabadoFormProps {
  acabado?: Acabado & { niveles_precio?: any[]; pasos?: any[] };
  onSubmit: (data: AcabadoFormData) => Promise<void>;
  onCancel: () => void;
}
...
export function AcabadoForm({ acabado, onSubmit, onCancel }: AcabadoFormProps) {
  ...
  const initialPasoRelacionado = acabado?.pasos?.[0];
  const initialCategorias = (acabado as any)?.acabados_categorias?.map((c: any) => c.categoria_id) || [];
  const [formData, setFormData] = useState<AcabadoFormData>({
    ...
    alcance: (acabado as any)?.alcance || 'por_item',
    tiene_niveles_precio: acabado?.tiene_niveles_precio || false,
    tipo_impacto: acabado?.tipo_impacto || null,
    valor_impacto: acabado?.valor_impacto || null,
    valor_impacto_secundario: acabado?.valor_impacto_secundario || null,
    paso_id: initialPasoRelacionado?.paso_id || null,
    niveles: acabado?.niveles_precio || [],
  });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-6">
        <div>
          ...
          <div className="space-y-4">
            <Input
              ...
              error={errors.nombre}
              required
              placeholder="Ej: Barnizado, Laminado, etc."
            />
        ...
        <div>
          ...
          <div className="space-y-4">
            <Switch
              label="Disponible como acabado independiente"
              description="Permite que este acabado se pueda ofrecer sin otros acabados"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Selecciona el paso asociado a este acabado
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
        ...
        <Button type="submit" variant="primary" isLoading={loading}>
          {acabado ? 'Actualizar Acabado' : 'Crear Acabado'}
        </Button>
      </div>
    </form>
  );
