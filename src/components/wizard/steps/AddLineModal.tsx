...
import { AlertCircle, Loader2, DollarSign, Package, Sparkles } from 'lucide-react';
...
export function AddLineModal({
  ...
  }, [existingLine, isOpen, selectedAcabados]);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditMode ? 'Editar Línea' : 'Agregar Línea'}
      size="md"
    >
      <div className="space-y-5">
        ...
        <div className="mt-4 bg-gray-900 rounded-xl p-4 text-white shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-y-1 text-sm mb-3">
            ...
            <div className="text-right font-medium">${(precioCalculado?.precio_base || 0).toFixed(2)}</div>

            {(precioCalculado?.precio_acabados || 0) > 0 && (
              <>
                <div className="text-gray-400">Acabados:</div>
                <div className="text-right font-medium text-purple-300">+${(precioCalculado?.precio_acabados || 0).toFixed(2)}</div>
              </>
            )}
          </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-700">
            ...
            <div className="text-xl font-bold text-white flex items-center">
              ...
              {(precioCalculado?.precio_total || 0).toFixed(2)}
            </div>
          </div>
        </div>
