...
import { calculateSharedServiceProration, getProrationDetails } from '../../utils/sharedServiceProration';
...
interface ServiciosAcabadosCompartidosSectionProps {
...
export function ServiciosAcabadosCompartidosSection({
  tipo,
  id,
  items,
  onTotalsChange
}: ServiciosAcabadosCompartidosSectionProps) {
  const {
  ...
  const handleDeleteAcabado = async (acabadoId: string) => {
    if (!confirm('¿Estás seguro de eliminar este acabado compartido?')) return;

    try {
      await deleteAcabadoCompartido(acabadoId);
      onTotalsChange?.();
    } catch (err) {
      console.error('Error deleting acabado:', err);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Servicios y acabados que aplican a toda la {tipo === 'orden' ? 'orden' : 'presupuesto'}
          </p>
        </div>
        {totalCompartidos > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          {serviciosCompartidos.length === 0 ? (
            ...
            <div className="space-y-3">
              {serviciosCompartidos.map((servicio: any) => (
                <div
                  key={servicio.id}
        ...
        <Card>
          ...
          {acabadosCompartidos.length === 0 ? (
            ...
            <div className="space-y-3">
              {acabadosCompartidos.map((acabado: any) => (
                <div
                  key={acabado.id}
                  className="p-4 bg-gray-50 rounded-lg border border-gray-200"
                >
