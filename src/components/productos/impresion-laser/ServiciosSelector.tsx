...
export function ServiciosSelector({
  ...
  const cargarServicios = async () => {
    try {
      ...
      if (servError) {
        console.error('[ServiciosSelector] Error:', servError);
        setServicios([]);
        return;
      }
      ...
      const serviciosConNiveles = await Promise.all(
        (serviciosData || []).map(async (servicio) => {
          if (servicio.tiene_niveles_precio) {
            ...
            if (nivelesError) {
              console.error('[ServiciosSelector] Error cargando niveles:', nivelesError);
      ...
      console.error('❌ [ServiciosSelector] Error cargando servicios:', err);
      setServicios([]);
    } finally {
      setIsLoading(false);
    }
