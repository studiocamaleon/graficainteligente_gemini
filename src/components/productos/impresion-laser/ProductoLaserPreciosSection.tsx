...
interface ProductoLaserPreciosSectionProps {
  productoId: string;
...
export function ProductoLaserPreciosSection({
  productoId,
  onPreciosChange,
}: ProductoLaserPreciosSectionProps) {
  const { producto, isLoading: loadingProducto } = useProductoImpresionLaser(productoId);
  const { precios: preciosExistentes, isLoading: loadingPrecios } = useProductosImpresionLaserPrecios(productoId);
