...
interface AcabadosSelectorGranFormatoProps {
  acabadosSeleccionados: string[];
  onChange: (acabados: string[]) => void;
  error?: string;
}

export function AcabadosSelectorGranFormato({
  acabadosSeleccionados,
  onChange,
  error,
}: AcabadosSelectorGranFormatoProps) {
  ...
  const cargarAcabados = async () => {
    try {
      ...
      if (relError) {
        console.error('[AcabadosSelectorGranFormato] Error:', relError);
        setAcabados([]);
        return;
      }
      ...
      if (acabError) {
        console.error('[AcabadosSelectorGranFormato] Error:', acabError);
        setAcabados([]);
        return;
      }
      ...
      console.error('❌ [AcabadosSelectorGranFormato] Error cargando acabados:', err);
      setAcabados([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="space-y-3">
        ...
        <div className="text-center py-6 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-500">
            No hay acabados disponibles para Gran Formato
          </p>
        </div>
      </div>
    );
