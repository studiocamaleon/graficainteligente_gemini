...
interface RangoAnilladoFormProps {
  rango?: CentroCopiadoRangoAnillado;
  onSubmit: (data: CentroCopiadoRangoAnilladoFormData) => Promise<void>;
  onCancel: () => void;
}

export function RangoAnilladoForm({ rango, onSubmit, onCancel }: RangoAnilladoFormProps) {
  const [formData, setFormData] = useState<CentroCopiadoRangoAnilladoFormData>({
    hojas_desde: 1,
    hojas_hasta: null,
    precio_ring_wire: 0,
    precio_plastico: 0,
  ...
  useEffect(() => {
    if (rango) {
      setFormData({
        ...
        precio_ring_wire: rango.precio_ring_wire,
        precio_plastico: rango.precio_plastico,
      });
  ...
  const validate = (): boolean => {
    ...
    if (formData.precio_ring_wire < 0) {
      newErrors.precio_ring_wire = 'El precio no puede ser negativo';
    }

    if (formData.precio_plastico < 0) {
      newErrors.precio_plastico = 'El precio no puede ser negativo';
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Input
          label="Precio Ring Wire"
          type="number"
          step="0.01"
          value={formData.precio_ring_wire}
          onChange={(e) => setFormData({ ...formData, precio_ring_wire: parseFloat(e.target.value) || 0 })}
          error={errors.precio_ring_wire}
          required
        />

        <Input
          label="Precio Plástico"
          type="number"
          step="0.01"
          value={formData.precio_plastico}
          onChange={(e) => setFormData({ ...formData, precio_plastico: parseFloat(e.target.value) || 0 })}
          error={errors.precio_plastico}
          required
        />
      </div>
