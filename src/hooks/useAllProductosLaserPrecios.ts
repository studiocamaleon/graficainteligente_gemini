...
export interface ProductoLaserParaPrecios {
  ...
  precios_existentes: ProductoLaserPrecio[];
}
...
export function useAllProductosLaserPrecios() {
  const [productos, setProductos] = useState<ProductoLaserParaPrecios[]>([]);
    ...
    try {
      ...
      const [tecnologiasRes, preciosRes, materialesRes] = await Promise.all([
        ...
        supabase
          ...
          .select('producto_laser_id, tecnologia_id, tintas, tecnologias(nombre)')
        ...
        supabase
          ...
          .select('producto_laser_id, materiales(nombre, unidad_espesor), variante_nombre, espesor')
          .in('producto_laser_id', productoIds),
      ]);
      ...
      (tecnologiasRes.data || []).forEach((tec: any) => {
        if (!tecnologiasByProducto.has(tec.producto_laser_id)) {
          tecnologiasByProducto.set(tec.producto_laser_id, []);
        }
        tecnologiasByProducto.get(tec.producto_laser_id)!.push(tec);
      });

      const preciosByProducto = new Map<string, ProductoLaserPrecio[]>();
      (preciosRes.data || []).forEach((precio: any) => {
        if (!preciosByProducto.has(precio.producto_laser_id)) {
          preciosByProducto.set(precio.producto_laser_id, []);
        }
        preciosByProducto.get(precio.producto_laser_id)!.push(precio);
      });

      const materialesByProducto = new Map<string, any[]>();
      (materialesRes.data || []).forEach((mat: any) => {
        if (!materialesByProducto.has(mat.producto_laser_id)) {
          materialesByProducto.set(mat.producto_laser_id, []);
        }
        materialesByProducto.get(mat.producto_laser_id)!.push(mat);
      });
