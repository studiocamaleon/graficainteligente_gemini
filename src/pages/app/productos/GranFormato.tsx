...
import { ProductosGranFormatoTab } from './gran-formato/ProductosGranFormatoTab';
import { PreciosGranFormatoTab } from './gran-formato/PreciosGranFormatoTab';

type TabType = 'productos' | 'precios';

export function GranFormato() {
  const [activeTab, setActiveTab] = useState<TabType>('productos');
  const [triggerCreate, setTriggerCreate] = useState(0);
  const { profile } = useAuth();
  const { canCreate } = usePermissions();

  const canCreateProduct = canCreate('productos-gran-formato');
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'productos':
        return <ProductosGranFormatoTab triggerCreate={triggerCreate} />;
      case 'precios':
        return <PreciosGranFormatoTab />;
      default:
        return null;
    }
  };
