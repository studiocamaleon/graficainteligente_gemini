...
import { useProductosGranFormato, useProductoGranFormato } from '../../../../hooks/useProductosGranFormato';
import { useConfirmDialog } from '../../../../hooks/useConfirmDialog';
import { useAuth } from '../../../../hooks/useAuth';
import { ProductoGranFormatoModal } from '../../../../components/productos/gran-formato/ProductoGranFormatoModal';
import { ProductoGranFormatoDetalle } from '../../../../components/productos/gran-formato/ProductoGranFormatoDetalle';
import type { ProductoGranFormato, ProductoGranFormatoConRelaciones } from '../../../../types/database';

interface ProductosGranFormatoTabProps {
  triggerCreate?: number;
}

export function ProductosGranFormatoTab({ triggerCreate = 0 }: ProductosGranFormatoTabProps) {
  ...
  const [selectedProducto, setSelectedProducto] = useState<ProductoGranFormatoConRelaciones | null>(null);
  ...
  const { productos, isLoading, refetch } = useProductosGranFormato(filters);
  const { toggleStatus, deleteProducto, producto: productoCompleto, refetch: refetchProducto, isLoading: isLoadingProducto } = useProductoGranFormato(selectedProducto?.id);
  ...
  const handleVerDetalle = async (producto: ProductoGranFormato) => {
    setSelectedProducto(producto as ProductoGranFormatoConRelaciones);
    await refetchProducto();
    setIsDetalleModalOpen(true);
  };

  const handleEditar = (producto: ProductoGranFormato) => {
    setSelectedProducto(producto as ProductoGranFormatoConRelaciones);
  ...
  const columns = [
    {
      key: 'nombre',
      header: 'Nombre',
      render: (producto: ProductoGranFormato) => (
            ...
            <EmptyState
              ...
              description={
                search || filterActive !== 'all'
                  ...
                  : 'Comienza creando tu primer producto de impresión gran formato'
              }
              action={
                !search && filterActive === 'all' ? (
