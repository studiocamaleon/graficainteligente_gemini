...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { cliente_id, whatsapp_backend_url } = await req.json() as NotifyRequest;
    ...
    const mensaje = `Hola ${cliente.nombre_fantasia}!
...
¡Bienvenido a ${company.name}!`;
    ...
    console.error('Error en notify-cliente-aprobado:', error);
    return new Response(
