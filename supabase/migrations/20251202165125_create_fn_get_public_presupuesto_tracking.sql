/*
  # Función para tracking público de presupuestos

  1. Nueva función
    - `fn_get_public_presupuesto_tracking`
    - Retorna información pública del presupuesto
    - Accesible sin autenticación mediante tracking_token

  2. Cambios
    - Función SECURITY DEFINER para permitir acceso público
    - Retorna datos seguros (sin info sensible interna)
*/

-- Función para obtener tracking público de presupuesto
...
COMMENT ON FUNCTION fn_get_public_presupuesto_tracking(varchar) IS
  'Obtiene información pública de un presupuesto mediante su tracking token';
