/*
  # Fix: Corregir nombres de columnas en fn_convertir_presupuesto_a_orden

  ## Problema
  La función usa nombres de columnas incorrectos al insertar rutas personalizadas:
  - Usa `item_id` pero la columna se llama `orden_item_id`
  - Usa `etapa` pero la columna se llama `tipo_etapa`
  - Usa `estado` pero esa columna no existe en ordenes_trabajo_items_rutas

  ## Solución
  Corregir los nombres de columnas en el INSERT de rutas personalizadas

  ## Cambios
  - Reemplazar `item_id` por `orden_item_id`
  - Reemplazar `etapa` por `tipo_etapa`
  - Eliminar columna `estado` (no existe en esta tabla)
*/
...
BEGIN
  ...
  VALUES (
    ...
    v_presupuesto.descripcion,
    v_fecha_entrega,
    COALESCE(p_notas_adicionales, v_presupuesto.notas),
    v_presupuesto.subtotal,
    v_presupuesto.impuestos,
    v_presupuesto.descuento,
    v_presupuesto.total,
    'pendiente',
    auth.uid()
  )
  ...
  LOOP
    ...
    ELSIF v_item.tipo_item = 'item_personalizado' AND p_rutas_personalizadas IS NOT NULL THEN
      ...
      IF v_rutas_item IS NOT NULL THEN
        ...
        LOOP
          -- Insertar con los nombres de columnas correctos
...
- Procesa rutas manuales para items personalizados (CORREGIDO)
- Opcionalmente registra un pago inicial
- Envía notificación WhatsApp de nueva orden
