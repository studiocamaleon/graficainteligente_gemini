-- =============================================
-- MIGRATION: 20251208001500_wip_states_fix.sql
-- Description: Expands WIP projection to include 'completado' and 'finalizada' states, ensuring finished but undelivered/unpaid orders are projected.
-- =============================================
...
BEGIN
    ...
    movements AS (
        ...
        -- Includes: Confirmado, En Produccion, Completado (Finished but not delivered/paid)
        SELECT 
            CASE 
        ...
        -- Includes: Pendiente, En Proceso, Finalizada (Finished but not delivered/paid)
        SELECT 
            CASE 
                WHEN fecha_entrega_estimada < CURRENT_DATE THEN CURRENT_DATE 
                ELSE fecha_entrega_estimada::date 
            END as fecha,
