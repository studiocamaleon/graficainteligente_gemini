-- Migration: Comprehensive Dynamic Step Lookup (Services, Finishes, Tech)
-- Date: 2025-12-15
-- Description: Updates fn_generar_ruta_produccion_item to include dynamic step lookup for:
-- 1. 'tecnologia_tinta' (using tecnologias_tintas_pasos)
-- 2. 'servicio_con_nivel' (using servicios_niveles_precio)
-- 3. 'acabado_con_nivel' (using acabados_niveles_precio)
-- This matches the full logic of the frontend's generateProductionRoutes.ts.
  ...
  LOOP
    ...
    CASE v_paso_record.tipo_condicion
      ...
      WHEN 'servicio_con_nivel' THEN
        -- Buscar el servicio que coincida con el ID y el nivel
        ...
        IF v_servicio_match IS NOT NULL THEN
          ...
          ELSE
             -- 2. Búsqueda dinámica en servicios_niveles_precio
             BEGIN
             ...
             EXCEPTION WHEN OTHERS THEN
               -- Si falla casting o tabla no existe, mantener original
               v_paso_especifico_id := v_paso_record.paso_id;
             END;
          END IF;
          
          -- Actualizar nombre si cambió el ID
      ...
      WHEN 'acabado_con_nivel' THEN
         -- Buscar el acabado que coincida con el ID y el nivel
        ...
        IF v_acabado_match IS NOT NULL THEN
          ...
          IF v_nivel_aplicado IS NOT NULL AND v_mapeo_niveles ? v_nivel_aplicado THEN
             v_paso_especifico_id := (v_mapeo_niveles->>v_nivel_aplicado)::uuid;
          ELSE
             -- 2. Búsqueda dinámica en acabados_niveles_precio
             BEGIN
               SELECT paso_id INTO v_paso_especifico_id
