-- Migration: Fix Cashflow Recurring Overdue
-- Description: Updates fn_get_cashflow_projection to include overdue recurring expenses (ghost debt) and sum them correctly.
...
RETURNS TABLE (
    ...
    -- OVERDUE METRICS (Included in totals, but isolated for display)
...
BEGIN
    ...
    movements AS (
        ...
        -- Logic mirrors fn_get_vencimientos_pendientes: look back 6 months for unpaid occurrences
        SELECT
            CURRENT_DATE as fecha, -- Map all past debt to Today
            re.amount as monto,
            'recurring_out' as type,
            true as is_overdue
        FROM recurring_expenses re
    ...
    daily_agg AS (
        SELECT 
            ...
            -- Overdue Metadata (Added recurring_out)
