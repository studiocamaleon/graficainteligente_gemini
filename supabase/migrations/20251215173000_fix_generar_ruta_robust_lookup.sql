-- Migration: Robust Route Lookup in fn_generar_ruta_produccion_item
-- Date: 2025-12-15
-- Description: Updates fn_generar_ruta_produccion_item to look for ruta_produccion_id in the dedicated sub-tables.
-- Handles legacy vs new category names.
...
BEGIN
  ...
  -- Fallback logic omitted as 'productos' table does not natively store ruta_produccion_id in this schema version
  -- (It is stored in the sub-tables via foreign keys added in migrations).
  ...
  LOOP
    ...
    CASE v_paso_record.tipo_condicion
      ...
      WHEN 'tecnologia_tinta' THEN
        -- Por ahora solo verificamos tecnología, la tinta se puede agregar después como refinamiento
        v_incluir_paso := (
