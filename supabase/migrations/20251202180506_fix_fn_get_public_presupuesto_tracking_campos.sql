/*
  # Fix fn_get_public_presupuesto_tracking - Campos Incorrectos

  1. Cambios
    - Eliminar campo `motivo_rechazo` (no existe en presupuestos)
    - Cambiar `cl.telefono` por `cl.whatsapp`
    - Cambiar campos de companies a nombres correctos:
      - `c.telefono` → `c.contact_phone`
      - `c.direccion` → `c.address`
      - `c.sitio_web` → `c.website`
      - `c.razon_social` → `c.legal_name`
    - Eliminar `ORDER BY pi.orden` (columna no existe)
    - Usar `created_at` para ordenar items
...
COMMENT ON FUNCTION fn_get_public_presupuesto_tracking(varchar) IS
  'Obtiene información pública de un presupuesto mediante su tracking token. Versión corregida con campos válidos.';
