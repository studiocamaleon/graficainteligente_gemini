-- BI v2 Gran Formato: filtro por tecnología + unidad tomada desde producto (fallback config).

DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_resumen_v2(uuid, date, date);
DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_resumen_v2(uuid, date, date, text);
...
$$;
GRANT EXECUTE ON FUNCTION public.fn_bi_productos_gran_formato_resumen_v2(uuid, date, date, text) TO authenticated;
...
DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_top_materiales_v2(uuid, date, date, integer, text);
...
lineas_metricas AS (
  SELECT
    COALESCE(NULLIF(CONCAT_WS(' - ', NULLIF(le.cfg->>'material_nombre', ''), NULLIF(le.cfg->>'variante_nombre', '')), ''), 'Sin material') AS material_label,
    (le.total_item / GREATEST(le.lineas_por_item, 1))::numeric AS ventas_linea,
    COALESCE(NULLIF(le.linea->>'cantidad', '')::numeric, NULLIF(le.cfg->>'cantidad', '')::numeric, le.unidades_item, 1)::numeric AS linea_cantidad,
    COALESCE(NULLIF(le.linea->>'ancho_seleccionado', '')::numeric, NULLIF(le.linea->>'ancho', '')::numeric, NULLIF(le.cfg->>'medida_ancho', '')::numeric, 0)::numeric AS ancho_cm,
    COALESCE(NULLIF(le.linea->>'metros_lineales', '')::numeric, CASE WHEN NULLIF(le.linea->>'alto', '') IS NOT NULL THEN NULLIF(le.linea->>'alto', '')::numeric / 100 ELSE NULL END, CASE WHEN NULLIF(le.cfg->>'medida_alto', '') IS NOT NULL THEN NULLIF(le.cfg->>'medida_alto', '')::numeric / 100 ELSE NULL END, 0)::numeric AS metros_lineales_base
  FROM lineas_expand le
),
agg AS (
  SELECT
...
SELECT a.material_label, a.total_ventas, a.total_lineas, a.total_mt2, a.total_ml
FROM agg a
ORDER BY a.total_ventas DESC
LIMIT GREATEST(COALESCE(p_limit, 10), 1);
$$;
GRANT EXECUTE ON FUNCTION public.fn_bi_productos_gran_formato_top_materiales_v2(uuid, date, date, integer, text) TO authenticated;

DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_anchos_v2(uuid, date, date, integer);
DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_anchos_v2(uuid, date, date, integer, text);
...
filtered AS (SELECT * FROM lineas_metricas WHERE ancho_cm > 0),
agg AS (
  SELECT
    f.ancho_cm,
...
$$;
GRANT EXECUTE ON FUNCTION public.fn_bi_productos_gran_formato_anchos_v2(uuid, date, date, integer, text) TO authenticated;

DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_tecnologia_unidad_v2(uuid, date, date);
DROP FUNCTION IF EXISTS public.fn_bi_productos_gran_formato_tecnologia_unidad_v2(uuid, date, date, text);
...
lineas_calc AS (
  SELECT
    le.tecnologia_item AS tecnologia_label,
    CASE
      WHEN COALESCE(le.tipo_venta_producto, lower(COALESCE(NULLIF(le.cfg->>'tipo_venta_real', ''), NULLIF(le.cfg->>'tipo_venta', '')))) = 'mt_lineal' THEN 'Metro lineal'
      WHEN COALESCE(le.tipo_venta_producto, lower(COALESCE(NULLIF(le.cfg->>'tipo_venta_real', ''), NULLIF(le.cfg->>'tipo_venta', '')))) = 'mt2' THEN 'Metro cuadrado'
...
ORDER BY a.total_ventas DESC;
$$;
GRANT EXECUTE ON FUNCTION public.fn_bi_productos_gran_formato_tecnologia_unidad_v2(uuid, date, date, text) TO authenticated;
