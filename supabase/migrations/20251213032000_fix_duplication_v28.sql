-- v28: Fix Mixed Order Duplication and Totals
-- Goal: 
-- 1. Eliminate duplication of items (CC Items must NOT go to OT).
-- 2. Correctly calculate totals for split orders (CC Total vs OT Total).
-- 3. Maintain V27 improvements (ID Generation, FKs).
...
BEGIN
  ...
  -- 2. Clasificar items y Calcular Totales Separados
  SELECT 
    count(*) FILTER (WHERE tipo_item = 'centro_copiado'),
    COALESCE(SUM(precio_total) FILTER (WHERE tipo_item = 'centro_copiado'), 0),
    count(*) FILTER (WHERE tipo_item != 'centro_copiado'),
    COALESCE(SUM(precio_total) FILTER (WHERE tipo_item != 'centro_copiado'), 0)
  INTO v_count_cc_items, v_total_cc, v_count_other_items, v_total_ot
  FROM presupuestos_items 
  WHERE presupuesto_id = p_presupuesto_id;

  -- Logic: If we rely on budget total (which includes discounts), we should assume:
  -- CC Total = Sum of items (usually net).
  -- OT Total = Budget Total - CC Total (allocates discounts to OT).
  -- Safety check to avoid negative.
  IF v_total_ot < 0 THEN v_total_ot := 0; END IF;
  ...
  -- HELPER: Generate CC Number if needed
  IF v_count_cc_items > 0 THEN
     v_cc_numero := generate_numero_orden_copiado(v_presupuesto.company_id);
  END IF;
  ...
  IF v_is_pure_cc THEN
     ...
     INSERT INTO centro_copiado_ordenes (
       ...
       v_cc_numero, 
       'pendiente', NOW(), p_fecha_entrega_estimada, v_presupuesto.total, -- Pure CC uses full budget total
  ...
  -- ==================================================================================
  ELSE
     -- Correct Total Logic for OT:
     -- Use v_total_ot (sum of its items) OR BudgetTotal - CCTotal.
     -- We use BudgetTotal - CCTotal to maintain consistency with global discounts applied to budget.
     -- If mixed, OT carries the burden of discounts.
     DECLARE
       v_final_ot_total numeric;
     BEGIN
       v_final_ot_total := v_presupuesto.total - v_total_cc;
       IF v_final_ot_total < 0 THEN v_final_ot_total := 0; END IF;
       ...
       INSERT INTO ordenes_trabajo (
         ...
         v_final_ot_total, -- CORRECTED TOTAL
         v_final_ot_total, -- CORRECTED SUBTOTAL (Approx)
     ...
     IF v_count_cc_items > 0 THEN
        INSERT INTO centro_copiado_ordenes (
          ...
          v_cc_numero,
          'pendiente', NOW(), p_fecha_entrega_estimada, 
          v_total_cc, -- CORRECTED TOTAL (Sum of CC items)
          COALESCE(p_notas_adicionales, v_presupuesto.notas_internas), v_origen,
          COALESCE(p_requiere_factura, false), v_origen, NOW(), NOW(), auth.uid()
        ) RETURNING id INTO v_cc_id;
     END IF;
     ...
     FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id LOOP
        IF v_item.tipo_item = 'centro_copiado' THEN
             ...
             END IF;
             -- DONT INSERT INTO OT ITEMS
        ELSE
           -- INSERT INTO OT ITEMS (Standard or Global Services)
