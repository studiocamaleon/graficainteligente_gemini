-- Migration: Fix Route Generation Stage Priority
-- Description: Updates fn_generar_ruta_produccion_item to prioritize the stage defined in the 'pasos' table
--              over the stage defined in the template wrapper. This fixes issues where template stages
--              might have loose or incorrect names (e.g. "Impresion UV") while the step is correctly
--              categorized (e.g. "Produccion").
...
DECLARE
  ...
  -- Nueva variable para asegurar la etapa correcta
  v_etapa_real text;
BEGIN
  ...
  LOOP
    ...
    v_etapa_real := NULL; -- Resetear para cada vuelta
    ...
    IF v_incluir_paso THEN
      -- CRITICO: Intentar buscar la etapa real del paso
      -- Si el paso_id específico tiene una etapa definida en 'pasos', esa es la autoridad.
      -- Si no (o si falla), hacemos fallback a v_paso_record.etapa (template).
      IF v_paso_especifico_id IS NOT NULL THEN
         BEGIN
           SELECT etapa INTO v_etapa_real FROM pasos WHERE id = v_paso_especifico_id;
         EXCEPTION WHEN OTHERS THEN
           v_etapa_real := NULL;
         END;
      END IF;
      ...
      INSERT INTO ordenes_trabajo_items_rutas (
        ...
        p_company_id, 
        p_orden_item_id, 
        COALESCE(v_etapa_real, v_paso_record.etapa), -- Prioridad a la etapa real del paso
        v_paso_especifico_id, 
        v_paso_nombre, 
        v_paso_record.orden, 
        false, 
        v_paso_record.id
