/*
  # Fix v5: Prioridad de Nombres y Limpieza de Rutas en Conversión
  
  ## Problemas Resueltos
  1. **Nombres Genéricos**: Anteriormente se priorizaba `descripcion` sobre `producto_nombre`. 
     Si `descripcion` era "Producto" (default), se perdía el nombre real (ej: "Laminado Brillante").
     Ahora se prioriza `producto_nombre`.
  
  2. **Rutas Vacías**: Se refuerza la lógica para asegurar que los servicios globales
     NO entren en la lógica de generación de rutas personalizadas (que podría generar estructuras vacías).
...
BEGIN
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      ELSE
        -- Log para debug
        RAISE NOTICE 'Skipping route generation for Global Service: %', v_item_descripcion;
