/*
  # Configurable Copy Center Routes

  1. New Table: `centro_copiado_rutas_configuracion`
     - Allows mapping configuration keys (e.g., 'anillado') + values (e.g., 'ring_wire') to specific Production Steps.
  
  2. Updated Function: `fn_generar_ruta_produccion_item`
     - Uses the new table to find steps.
     - Fallback to legacy "fuzzy match" logic if no configuration found.
     - Adds support for 'dobladillo' (previously missing).
*/

-- 1. CREATE CONFIGURATION TABLE
CREATE TABLE IF NOT EXISTS centro_copiado_rutas_configuracion (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  clave text NOT NULL, -- e.g., 'anillado', 'plastificado'
  valor text, -- e.g., 'ring_wire', 'A4', or NULL for wildcard
  paso_id uuid NOT NULL REFERENCES pasos(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT centro_copiado_rutas_config_unique UNIQUE NULLS NOT DISTINCT(company_id, clave, valor)
);

-- RLS
ALTER TABLE centro_copiado_rutas_configuracion ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own company route configs"
  ON centro_copiado_rutas_configuracion FOR SELECT
  TO authenticated
  USING (company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid()));

CREATE POLICY "Users can manage own company route configs"
  ON centro_copiado_rutas_configuracion FOR ALL
  TO authenticated
  USING (company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid()))
  WITH CHECK (company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid()));

-- Index for fast lookup
CREATE INDEX IF NOT EXISTS idx_centro_copiado_rutas_config_lookup 
  ON centro_copiado_rutas_configuracion(company_id, clave, valor);


-- 2. UPDATE ROUTE GENERATION FUNCTION
