/*
  # Fix: Corregir llamada a fn_generar_ruta_produccion_item
  
  ## Problema
  La función fn_convertir_presupuesto_a_orden llama a fn_generar_ruta_produccion_item
  con solo 3 parámetros, pero la función espera 5 parámetros:
  - p_orden_item_id
  - p_producto_id
  - p_categoria (FALTA)
  - p_configuracion
  ...
  Esto causa que falle la generación de rutas y los productos del catálogo
  no tengan rutas de producción al convertir presupuestos a órdenes.
  
  ## Solución
  Actualizar la llamada para incluir los 5 parámetros correctos:
  - Agregar v_item.producto_categoria
  - Agregar v_presupuesto.company_id
  
  ## Cambios
  - Actualizar fn_convertir_presupuesto_a_orden con llamada correcta
*/
    ...
    IF v_item.tipo_item = 'producto_sistema' AND v_item.producto_id IS NOT NULL THEN
      BEGIN
        ...
        SELECT fn_generar_ruta_produccion_item(
          v_nuevo_item_id,              -- p_orden_item_id
          v_item.producto_id,            -- p_producto_id
          v_item.producto_categoria,     -- p_categoria
          v_item.configuracion,          -- p_configuracion
          v_presupuesto.company_id       -- p_company_id
...
'Convierte presupuesto aprobado a orden de trabajo con rutas de producción.
