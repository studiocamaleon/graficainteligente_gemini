-- v21: Restore Copy Center Order Logic
-- Goal: Restore creation of 'centro_copiado_ordenes' for Copy Center budgets.
-- Logic:
-- 1. Check if budget has 'centro_copiado' items.
-- 2. If yes, create record in 'centro_copiado_ordenes' linked to 'ordenes_trabajo'.
-- 3. Populate 'centro_copiado_ordenes_items' from budget items definition.
-- 4. Return correct ID (CC Order ID if CC budget, Work Order ID otherwise).
...
DECLARE
  ...
  v_orden_id uuid;            -- Orden de Trabajo ID
  v_cc_orden_id uuid;         -- Centro Copiado Orden ID
  v_item RECORD;
...
BEGIN
  ...
  -- Detectar si es Presupuesto de Centro de Copiado
  SELECT 
    count(*), 
  ...
  -- Logic: If ALL items are Copy Center, treat as CC Order
  ...
  -- 2. Crear Orden de Trabajo (Always created as parent/master)
  ...
  -- 3. Si es Centro de Copiado, Crear Orden Específica
  IF v_es_centro_copiado THEN
     INSERT INTO centro_copiado_ordenes (
       company_id,
       cliente_id,
       orden_trabajo_id, -- Link to parent
       numero_orden,
       estado,
       fecha_solicitud,
       ...
       v_presupuesto.cliente_id,
       v_orden_id,
       -- Use generated number logic or same as OT? Usually has its own sequence or copy
  ...
  -- 4. Pagos (Link mainly to Orden Trabajo, or both? Standard is OT handles payments)
  ...
  LOOP
    -- A) Si es item de Centro de Copiado, insertar en su tabla específica
    IF v_item.tipo_item = 'centro_copiado' AND v_es_centro_copiado THEN
       ...
       -- ALSO Insert into Main Items table (as 'personalizado' via V20 logic) so it appears in main lists?
       ...
       INSERT INTO ordenes_trabajo_items (
         ...
         v_orden_id,
         'personalizado', -- Safe map for CC items
         NULL,
         v_item.descripcion,
         v_item.cantidad,
    ...
    ELSE
       -- B) Items Normales (Logic from V20)
        IF v_item.configuracion->>'es_servicio_global' = 'true' THEN
           v_tipo_item_orden := 'servicio';
        ...
        ELSE
           INSERT INTO ordenes_trabajo_items (
             ...
             v_orden_id, v_tipo_item_orden, v_item.producto_id, v_item.descripcion,
             v_item.cantidad, v_item.configuracion, v_item.precio_unitario_final,
             v_item.precio_total, v_item.producto_nombre, v_item.producto_categoria,
           ...
           IF v_tipo_item_orden = 'catalogo' THEN
              ...
              EXCEPTION WHEN OTHERS THEN RAISE WARNING 'Error rutas: %', SQLERRM; END;
  ...
  EXCEPTION WHEN OTHERS THEN RAISE WARNING 'Error notifying: %', SQLERRM; END;

  -- 9. Return ID (Critical: Return CC ID if valid, else Work Order ID)
  IF v_es_centro_copiado AND v_cc_orden_id IS NOT NULL THEN
     RETURN v_cc_orden_id;
  ELSE
     RETURN v_orden_id;
  END IF;
