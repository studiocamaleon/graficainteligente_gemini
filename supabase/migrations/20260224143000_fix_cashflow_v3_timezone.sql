-- Cashflow V3 timezone hardening
-- Canonical timezone for date boundaries and overdue logic

DROP FUNCTION IF EXISTS fn_get_cashflow_projection_v3(UUID, INTEGER, TEXT, JSONB);
DROP FUNCTION IF EXISTS fn_get_cashflow_projection_v3(UUID, INTEGER, TEXT, JSONB, TEXT);
...
DECLARE
    ...
    v_end_date DATE;
    v_timezone TEXT := COALESCE(NULLIF(p_timezone, ''), 'America/Argentina/Buenos_Aires');
    v_today_ar DATE := (now() AT TIME ZONE v_timezone)::date;
    v_window_start_recurring DATE := (v_today_ar - INTERVAL '6 months')::date;
    v_basis TEXT := CASE
...
BEGIN
    ...
    v_end_date := v_today_ar + p_days_to_project;

    RETURN QUERY
    WITH calendar AS (
        SELECT i::date as fecha
        FROM generate_series(v_today_ar, v_end_date, '1 day'::interval) i
    ),
    movements AS (
        SELECT
            GREATEST(fecha_pago::date, v_today_ar) as fecha,
            monto as monto,
            'cheque_in'::text as type,
            (fecha_pago::date < v_today_ar) as is_overdue
        ...
        SELECT
            GREATEST(fecha_vencimiento::date, v_today_ar) as fecha,
            (total_consumos - total_pagado) as monto,
            'tarjeta_out'::text as type,
            (fecha_vencimiento::date < v_today_ar) as is_overdue
        ...
        CROSS JOIN LATERAL (
            ...
            FROM generate_series(GREATEST(re.start_date, v_window_start_recurring), v_today_ar - INTERVAL '1 day', '1 day'::interval) d
            WHERE
        ...
        SELECT
            GREATEST(COALESCE(fecha_estimada_entrega, v_today_ar)::date, v_today_ar) as fecha,
            GREATEST(0, (total - COALESCE((SELECT SUM(monto) FROM ordenes_trabajo_pagos WHERE orden_id = ot.id), 0))) as monto,
            CASE
              WHEN COALESCE(fecha_estimada_entrega, v_today_ar)::date < v_today_ar THEN 'wip_overdue'::text
              ELSE 'wip_future'::text
            END as type,
            (COALESCE(fecha_estimada_entrega, v_today_ar)::date < v_today_ar) as is_overdue
        FROM ordenes_trabajo ot
        WHERE company_id = p_company_id
          AND estado NOT IN ('borrador', 'cotizacion', 'cancelada')
          AND (
            COALESCE(fecha_estimada_entrega, v_today_ar)::date >= v_today_ar
            OR v_basis = 'total'
        ...
        SELECT
            GREATEST(COALESCE(fecha_entrega_estimada, v_today_ar)::date, v_today_ar) as fecha,
            GREATEST(0, (total - COALESCE((SELECT SUM(monto) FROM centro_copiado_ordenes_pagos WHERE orden_copiado_id = cco.id), 0))) as monto,
            CASE
              WHEN COALESCE(fecha_entrega_estimada, v_today_ar)::date < v_today_ar THEN 'wip_overdue'::text
              ELSE 'wip_future'::text
            END as type,
            (COALESCE(fecha_entrega_estimada, v_today_ar)::date < v_today_ar) as is_overdue
        FROM centro_copiado_ordenes cco
        WHERE company_id = p_company_id
          AND estado NOT IN ('cancelada')
          AND orden_trabajo_id IS NULL
          AND (
            COALESCE(fecha_entrega_estimada, v_today_ar)::date >= v_today_ar
            OR v_basis = 'total'
            OR estado IN ('finalizada', 'entregada')
          )
          AND NOT EXISTS (
...
GRANT EXECUTE ON FUNCTION fn_get_cashflow_projection_v3(UUID, INTEGER, TEXT, JSONB, TEXT) TO authenticated;
