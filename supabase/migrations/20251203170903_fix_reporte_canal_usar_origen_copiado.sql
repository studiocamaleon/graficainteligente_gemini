/*
  # Fix: Usar campo 'origen' de órdenes de copiado en reportes por canal

  ## Descripción
  Corrige la función `fn_reporte_ventas_por_canal` para que use el campo `origen`
  de las órdenes de centro copiado en lugar de hardcodear 'Mostrador'.

  ## Problema
  La función estaba forzando todas las órdenes de copiado independientes a aparecer
  como canal 'Mostrador', ignorando el valor real del campo `origen` que puede ser
  'Web', 'WhatsApp', 'Mostrador' o 'App Mobile'.

  ## Solución
  - Para órdenes de copiado vinculadas: usar canal de orden de trabajo, pero si es NULL, usar origen de copiado
  - Para órdenes de copiado independientes: usar el campo `origen` directamente
  - Mantener 'Mostrador' como fallback solo si ambos son NULL

  ## Impacto
  - Las órdenes de copiado con origen 'App Mobile' o 'WhatsApp' ahora se reportarán correctamente
  - Los reportes de ventas por canal serán más precisos
*/
