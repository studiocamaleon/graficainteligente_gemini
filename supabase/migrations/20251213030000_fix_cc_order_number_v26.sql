-- v26: Correct Order Number Generation for Copy Center
-- Goal: Ensure Copy Center orders get "CC-YYYYMMDD-XXXX" format instead of "GI-XXXX".
-- Logic: Implement custom number generation for CC orders within the specific branches.
...
DECLARE
  ...
  -- Variables for CC Number Generation
  v_cc_numero text;
  v_date_str text;
...
BEGIN
  ...
  -- HELPER LOGIC: Generate CC Number if needed
  -- Only generate if we are going to create a CC order
  IF v_count_cc_items > 0 THEN
     ...
     v_cc_numero := 'CC-' || v_date_str || '-' || LPAD((v_daily_count + 1)::text, 4, '0');
  END IF;
  ...
  IF v_is_pure_cc THEN
     ...
     INSERT INTO centro_copiado_ordenes (
       ...
       v_cc_numero, -- FIXED: Use custom CC number
  ...
  ELSE
     ...
     INSERT INTO ordenes_trabajo (
       ...
       generate_numero_orden(v_presupuesto.company_id), -- Kept standard for OT
       NOW(), NOW(), auth.uid()
     ...
     IF v_count_cc_items > 0 THEN
        INSERT INTO centro_copiado_ordenes (
          ...
          v_cc_numero, -- FIXED: Use custom CC number here too
          'pendiente', NOW(), p_fecha_entrega_estimada, v_presupuesto.total,
          COALESCE(p_notas_adicionales, v_presupuesto.notas_internas), v_origen,
          COALESCE(p_requiere_factura, false), v_origen, NOW(), NOW(), auth.uid()
        ) RETURNING id INTO v_cc_id;
