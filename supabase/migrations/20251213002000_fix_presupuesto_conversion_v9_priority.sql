-- v9: Reorder Name Resolution Priority
-- Goal: Prefer Catalog Product Name over generic Description when item name is "Producto".
...
BEGIN
  ...
  INSERT INTO ordenes_trabajo (
    ...
    'presupuesto',
    p_presupuesto_id,
    0 -- Se calculará después o via triggers
  ...
  LOOP
    -- Determinar el tipo de item para la orden
    ...
    -- ==================================================================================
    -- ESTRATEGIA DE RESOLUCION DE NOMBRE (V9 - PRIORIDAD CATALOGO SOBRE DESCRIPCION)
    -- ==================================================================================
    v_final_name := NULL;

    -- 1. Intentar desde Configuración (JSON) - Máxima prioridad (lo que el usuario vio/editó)
    ...
    -- 3. (MOVIDO) Intentar desde CATALOGO (Productos) usando ID
    -- Si seguimos con NULL o "Producto", buscamos el nombre real del producto en la base de datos
    -- Esto tiene prioridad sobre una descripcion generica.
    ...
    -- 4. (FALLBACK) Intentar desde columna descripcion
    -- Solo si aun no tenemos nombre (o es Producto), usamos la descripcion si es corta.
    -- O si no tenemos producto_id.
    IF v_final_name IS NULL OR v_final_name ILIKE 'Producto' THEN
    ...
    -- Insertar Item
    -- LOGICA DE SERVICIOS GLOBALES (V7)
    ...
    ELSE
       ...
       INSERT INTO ordenes_trabajo_items (
         ...
         tipo_item,
         producto_id,
         descripcion, -- Aqui va el nombre principal
         cantidad,
         medidas,
         configuracion,
         archivos_adjuntos,
         ...
         '[]'::jsonb, -- Placeholder archivos
         v_item.rutas_generadas, -- Rutas del presupuesto si existen
         v_item.precio_unitario_final,
         v_item.precio_total,
         'pendiente'
       );
  ...
  -- 4. Actualizar estado presupuesto
  UPDATE presupuestos
