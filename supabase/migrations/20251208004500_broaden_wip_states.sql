-- =============================================
-- MIGRATION: 20251208004500_broaden_wip_states.sql
-- Description: Switches WIP projection filters to a "Blacklist" approach (excluding only known non-active states) 
-- to ensure ALL active orders with pending balances are captured, regardless of status nuances.
-- =============================================
...
BEGIN
    ...
    movements AS (
        ...
        -- Strategy: Include ALL orders that are NOT draft/quote/cancelled
        SELECT 
        ...
        WHERE company_id = p_company_id
          AND estado NOT IN ('borrador', 'cotizacion', 'cancelado') -- Blacklist approach
          AND NOT EXISTS (
        ...
        -- Strategy: Include ALL orders that are NOT cancelled
        SELECT 
        ...
        WHERE company_id = p_company_id
          AND estado NOT IN ('cancelada') -- Blacklist approach: Include 'entregada' if unpaid!
...
-- Update Debug Function to match
