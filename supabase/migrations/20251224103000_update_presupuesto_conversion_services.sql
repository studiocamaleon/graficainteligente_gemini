/*
  # Update Budget Conversion for Services

  Updates `fn_convertir_presupuesto_a_orden` to:
  1. Copy services from `presupuestos_servicios` to `ordenes_trabajo_servicios`.
  2. Handle the new structure where services are not mixed with items.
*/
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id LOOP
    ...
    FOR v_ruta_db IN SELECT * FROM presupuestos_items_rutas WHERE presupuesto_item_id = v_item.id ORDER BY orden LOOP
      ...
      VALUES (
        ...
        CASE
          WHEN LOWER(v_ruta_db.tipo_etapa) LIKE '%inst%' THEN 'instalacion'
          WHEN LOWER(v_ruta_db.tipo_etapa) LIKE '%post%' OR LOWER(v_ruta_db.tipo_etapa) LIKE '%term%' THEN 'post_prensa'
          WHEN LOWER(v_ruta_db.tipo_etapa) LIKE 'pre%' OR LOWER(v_ruta_db.tipo_etapa) LIKE '%pre-%' OR LOWER(v_ruta_db.tipo_etapa) = 'pre_prensa' THEN 'pre_prensa'
          ELSE 'principal' -- Fallback para 'produccion', nulls, others
        END,
        v_ruta_db.paso_id,
        COALESCE(v_ruta_db.paso_nombre, 'Paso'),
        COALESCE(v_ruta_db.orden, 0),
    ...
    IF NOT v_has_db_routes AND p_rutas_personalizadas IS NOT NULL AND (p_rutas_personalizadas->v_item.id::text) IS NOT NULL THEN
        ...
        FOR v_ruta_json IN SELECT * FROM jsonb_array_elements(v_rutas_item) LOOP
          ...
          VALUES (
            v_nuevo_item_id, v_presupuesto.company_id,
            CASE
              WHEN LOWER((v_ruta_json.value->>'etapa')::text) LIKE '%inst%' THEN 'instalacion'
              WHEN LOWER((v_ruta_json.value->>'etapa')::text) LIKE '%post%' OR LOWER((v_ruta_json.value->>'etapa')::text) LIKE '%term%' THEN 'post_prensa'
              WHEN LOWER((v_ruta_json.value->>'etapa')::text) LIKE 'pre%' OR LOWER((v_ruta_json.value->>'etapa')::text) LIKE '%pre-%' OR LOWER((v_ruta_json.value->>'etapa')::text) = 'pre_prensa' THEN 'pre_prensa'
              ELSE 'principal'
            END,
            (v_ruta_json.value->>'paso_id')::uuid,
            COALESCE((v_ruta_json.value->>'paso_nombre')::text, 'Paso Personalizado'), 
            (v_ruta_json.value->>'orden')::integer,
  ...
  -- 5.1 Copiar Servicios Adicionales (Desde tabla separada)
  FOR v_servicio IN SELECT * FROM presupuestos_servicios WHERE presupuesto_id = p_presupuesto_id LOOP
