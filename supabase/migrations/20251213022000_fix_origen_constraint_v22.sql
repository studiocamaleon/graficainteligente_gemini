-- v22: Fix Origen Constraint
-- Goal: Update 'centro_copiado_ordenes' insert to use a valid 'origen' value.
-- Analysis: 'presupuesto' failed constraint "centro_copiado_ordenes_origen_check".
-- Solution: Switch 'origen' to 'mostrador', which is a standard allowed value, or 'web' if applicable.
-- Given 'canal_venta' is also used, 'origen' might be legacy or redundant, but must pass check.
-- We will use 'mostrador' as safest fallback.
...
DECLARE
  ...
  v_es_centro_copiado boolean := false;
  v_count_cc_items integer := 0;
  v_count_all_items integer := 0;
BEGIN
  -- Validar estado
  ...
  -- Detectar Centro de Copiado
  SELECT count(*), count(*) FILTER (WHERE tipo_item = 'centro_copiado')
  INTO v_count_all_items, v_count_cc_items
  FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id;

  IF v_count_all_items > 0 AND v_count_all_items = v_count_cc_items THEN
     v_es_centro_copiado := true;
  END IF;
  ...
  INSERT INTO ordenes_trabajo (
    ...
    COALESCE(v_presupuesto.canal_venta, 'Mostrador'), p_fecha_entrega_estimada,
  ...
  -- Crear Orden Centro Copiado (Si aplica)
  IF v_es_centro_copiado THEN
     INSERT INTO centro_copiado_ordenes (
       ...
       total,
       observaciones,
       origen,  -- CHANGED FROM 'presupuesto' TO 'mostrador' to pass constraint
       requiere_factura,
       canal_venta,
       created_at,
       updated_at,
       ...
       v_orden_id,
       (SELECT numero_orden FROM ordenes_trabajo WHERE id = v_orden_id), 
       'pendiente',
       NOW(),
       p_fecha_entrega_estimada,
       v_presupuesto.total,
       COALESCE(p_notas_adicionales, v_presupuesto.notas_internas),
       'mostrador', -- FIXED: Using 'mostrador' as safe value
       COALESCE(p_requiere_factura, false),
       COALESCE(v_presupuesto.canal_venta, 'Mostrador'),
       NOW(),
       NOW(),
       auth.uid()
     ) RETURNING id INTO v_cc_orden_id;
  END IF;

  -- Pagos
  IF p_monto_pago IS NOT NULL AND p_monto_pago > 0 AND p_medio_cobro_id IS NOT NULL THEN
    INSERT INTO ordenes_trabajo_pagos (
      orden_id, medio_cobro_id, monto, referencia_pago, notas, fecha_pago, created_by
    ) VALUES (
      v_orden_id, p_medio_cobro_id, p_monto_pago, p_referencia_pago, 'Pago inicial', CURRENT_DATE, auth.uid()
    );
  END IF;

  -- Items Loop
  FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id LOOP
    IF v_item.tipo_item = 'centro_copiado' AND v_es_centro_copiado THEN
       -- Insert in CC Items
       ...
       -- Insert in Main Items (linked for consistency)
       INSERT INTO ordenes_trabajo_items (
         ...
         v_orden_id, 'personalizado', NULL, v_item.descripcion, v_item.cantidad, v_item.configuracion,
    ...
    ELSE
       -- Items Normales / Servicios Globales
        IF v_item.configuracion->>'es_servicio_global' = 'true' THEN
           INSERT INTO ordenes_trabajo_servicios (
             ...
             v_orden_id, (v_item.configuracion->>'servicio_id')::uuid,
        ...
        ELSE
           ...
           INSERT INTO ordenes_trabajo_items (
             ...
             v_orden_id, v_tipo_item_orden, v_item.producto_id, v_item.descripcion, v_item.cantidad, v_item.configuracion,
             v_item.precio_unitario_final, v_item.precio_total, v_item.producto_nombre, v_item.producto_categoria,
             'pendiente', v_item.precio_unitario_final, v_item.precio_servicios,
             v_item.precio_acabados, v_item.tiempo_produccion_dias, NOW(), NOW()
  ...
  SELECT v_orden_id, company_id, nombre_archivo, nombre_storage, tipo_mime, tamano_bytes, storage_path, descripcion, uploaded_by FROM presupuestos_archivos WHERE presupuesto_id = p_presupuesto_id;

  UPDATE presupuestos SET estado = 'convertido', orden_trabajo_id = v_orden_id, updated_at = NOW() WHERE id = p_presupuesto_id;
  ...
  IF v_es_centro_copiado AND v_cc_orden_id IS NOT NULL THEN RETURN v_cc_orden_id; ELSE RETURN v_orden_id; END IF;
