-- Migration to fix fn_convertir_presupuesto_a_orden (v4)
-- Fixes:
-- 1. Removes usage of non-existent 'orden' column
-- 2. Calculates and updates Total for Copy Center Orders
-- 3. Uses correct 'precio_unitario_final' mapping
-- 4. Uses correct 'pendiente' initial status
-- 5. Uses correct 'CC-...' ID generation
...
BEGIN
  ...
  ELSE
    ...
    IF v_count_copiado > 0 THEN
       v_numero_orden_copiado := generate_numero_orden_copiado(v_presupuesto.company_id);
       v_total_copiado := 0; -- Reset for mixed scenario
    ...
    -- Removed ORDER BY orden to avoid error
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
