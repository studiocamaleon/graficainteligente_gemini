/*
  # Agregar notificación WhatsApp al convertir presupuesto
  
  ## Problema
  Cuando se convierte un presupuesto en orden de trabajo, no se envía
  notificación WhatsApp al cliente informándole de su nueva orden.
  
  ## Solución
  Modificar fn_convertir_presupuesto_a_orden para que:
  1. Registre una notificación pendiente en whatsapp_notificaciones
  2. Llame a la edge function para enviar el mensaje
  
  ## Mensaje que se enviará
  Similar al de creación de orden normal:
  - Número de orden (GI-XXXX)
  - Total
  - Link de tracking
  - Agradecimiento
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Generar ruta de producción solo para items de catálogo
    IF v_item.tipo_item = 'catalogo' AND v_item.producto_id IS NOT NULL THEN
      BEGIN
      ...
      END;
    ELSIF v_item.tipo_item = 'personalizado' THEN
      RAISE NOTICE 'Item personalizado %: rutas deben configurarse manualmente', v_nuevo_item_id;
    END IF;
  END LOOP;
...
Soporta items personalizados y envía notificación WhatsApp al cliente.';
