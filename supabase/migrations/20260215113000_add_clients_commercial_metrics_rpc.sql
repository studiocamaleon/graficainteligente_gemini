-- Commercial client intelligence metrics for Clients module
-- Includes recency, frequency_90d, avg ticket, preferred channel, OT/CC mix and risk label.

CREATE INDEX IF NOT EXISTS idx_ordenes_trabajo_company_cliente_fecha_estado
  ON public.ordenes_trabajo(company_id, cliente_id, fecha_creacion, estado);

CREATE INDEX IF NOT EXISTS idx_centro_copiado_company_cliente_fecha_estado
  ON public.centro_copiado_ordenes(company_id, cliente_id, created_at, estado);
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      CASE
        WHEN ac.ultima_compra IS NULL THEN 'alto'
...
GRANT EXECUTE ON FUNCTION public.fn_list_clients_commercial_metrics(uuid, text, boolean, boolean, text, text, integer, integer, text, integer) TO authenticated;
