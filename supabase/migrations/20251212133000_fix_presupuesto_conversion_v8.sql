-- Migration to fix fn_convertir_presupuesto_a_orden (v8)
-- Fixes:
-- 1. TAX EXTRACTION: Divides budget amounts by 1.21 to extract Net Base.
--    This prevents double taxation when the Order system adds its own 21% VAT.
-- 2. NAME MAPPING: Ensures 'producto_nombre' is visible in OT items 'descripcion'.
-- 3. ROUTE GENERATION: Correctly calls fn_generar_ruta_produccion_item with all 5 required arguments.
-- 4. SCHEMA: Maintains v6 rigorous column mapping.
...
DECLARE
  ...
  v_tax_divisor numeric := 1.21; -- Constant to extract Tax Base (Net)
BEGIN
  ...
  IF v_count_sistema = 0 AND v_count_personalizado = 0 AND v_count_copiado > 0 THEN
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item = 'centro_copiado' LOOP
      -- Accumulate total (Store NET in OC total so system can add tax if integrated, though for standalone it defines the price)
      v_total_copiado := v_total_copiado + (v_item.precio_total / v_tax_divisor);
      ...
      INSERT INTO centro_copiado_ordenes_items (
        ...
        v_item.cantidad,
        v_item.precio_unitario_final / v_tax_divisor, -- NET
        v_item.precio_total / v_tax_divisor, -- NET
        v_item.descripcion
      );
    END LOOP;
  ...
  ELSE
    ...
    INSERT INTO ordenes_trabajo (
      ...
      v_total_ot / v_tax_divisor, -- Insert NET Base as Subtotal
      0,          
      v_total_ot, -- Insert GROSS as Total (Initial value)
      'pendiente',
      auth.uid()
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      INSERT INTO ordenes_trabajo_items (
        ...
        v_item.cantidad,
        v_item.precio_base / v_tax_divisor, -- NET
        v_item.precio_servicios / v_tax_divisor, -- NET
        v_item.precio_acabados / v_tax_divisor, -- NET
        v_item.precio_unitario_final / v_tax_divisor, -- NET 
        v_item.precio_total / v_tax_divisor, -- NET
      ...
      -- Generar rutas
      -- CORRECTED: Pass all 5 arguments defined in fn_generar_ruta_produccion_item
      IF v_item.tipo_item = 'producto_sistema' AND v_item.producto_id IS NOT NULL THEN
        BEGIN
          SELECT fn_generar_ruta_produccion_item(
            v_nuevo_item_id,
            v_item.producto_id,
            v_item.producto_categoria, -- Param 3: Category
            COALESCE(v_item.configuracion, '{}'::jsonb), -- Param 4: Config
            v_presupuesto.company_id -- Param 5: Company
