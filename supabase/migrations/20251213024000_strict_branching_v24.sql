-- v24: Strict Branching for Order Types
-- Goal: Strictly separate order creation logic based on content.
-- Rule 1: Only Copy Center items -> Create ONLY centro_copiado_ordenes (Link budget to it).
-- Rule 2: Only Catalog/Custom items -> Create ONLY ordenes_trabajo.
-- Rule 3: Mixed -> Create ordenes_trabajo AND linked centro_copiado_ordenes.
...
DECLARE
  ...
  v_orden_id uuid;            -- ID final a retornar
  v_ot_id uuid;               -- Orden Trabajo ID (si aplica)
  v_cc_id uuid;               -- Centro Copiado ID (si aplica)
  v_item RECORD;
  ...
  v_count_other_items integer := 0; -- Catalog, Custom, etc. (excluding global services usually?)
                                    -- Wait, global services shouldn't dictate order type? 
                                    -- Assuming global services go with OT.
...
BEGIN
  ...
  -- Determinar si es Puro Centro Copiado
  -- (Si solo tiene items de copiado y NO tiene items normales)
  -- Nota: Servicios Globales suelen acompañar a items. Si hay solo servicios globales y CC, qué pasa?
  -- Asumiremos que si v_count_other_items == 0, es Pure CC.
  ...
  -- ==================================================================================
  -- BRANCH A: PURE COPY CENTER (Solo crea CC Order)
  -- ==================================================================================
  IF v_is_pure_cc THEN
     ...
     INSERT INTO centro_copiado_ordenes (
       company_id,
       cliente_id,
       orden_trabajo_id, -- NULL porque es pura
       numero_orden,
       estado,
       fecha_solicitud,
       ...
       NULL, -- No parent OT
       generate_numero_orden(v_presupuesto.company_id), -- Usa generador global o propio de CC? 
                                                        -- El usuario dijo "CC-XXXX". El generador actual hace GI-XXXX?
                                                        -- Asumiremos que generate_numero_orden sirve, o deberíamos ver si hay otro.
                                                        -- Por ahora usamos el standard.
       'pendiente',
       NOW(),
     ...
     v_orden_id := v_cc_id; -- Retornaremos este ID
     ...
     -- Copiar Archivos a CC Archivos
     -- Tabla target: centro_copiado_ordenes_archivos (seguro existe si hay ordenes CC)
     -- Columns: orden_copiado_id, company_id, nombre_archivo, nombre_storage, ...
     ...
     -- Update Presupuesto (Guardamos el ID de CC en orden_trabajo_id? Es un UUID, cabe.)
     UPDATE presupuestos SET estado = 'convertido', orden_trabajo_id = v_cc_id, updated_at = NOW() WHERE id = p_presupuesto_id;

  -- ==================================================================================
  -- BRANCH B: STANDARD OR MIXED (Crea OT, y opcionalmente CC ligada)
  -- ==================================================================================
  ELSE
     -- Crear OT
     ...
     -- Si es Mixto (tiene items CC), crear CC Order ligada
     IF v_count_cc_items > 0 THEN
        INSERT INTO centro_copiado_ordenes (
          ...
          v_presupuesto.company_id, v_presupuesto.cliente_id, v_ot_id, -- LINKED
          (SELECT numero_orden FROM ordenes_trabajo WHERE id = v_ot_id), 
          'pendiente', NOW(), p_fecha_entrega_estimada, v_presupuesto.total, -- Total? Should be partial? User said "OC asociada". usually copies total or 0.
     ...
     FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id LOOP
        ...
        -- Si es CC Item -> Insertar en CC (si existe tabla items cc) y opcionalmente en OT como 'personalizado' para billing?
        IF v_item.tipo_item = 'centro_copiado' THEN
             ...
             -- Insert en OT Items (para facturacion global)
     ...
     UPDATE presupuestos SET estado = 'convertido', orden_trabajo_id = v_ot_id, updated_at = NOW() WHERE id = p_presupuesto_id;
