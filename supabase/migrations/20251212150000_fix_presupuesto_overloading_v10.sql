-- Migration v10: Resolving PGRST203 (Function Overloading) Error
-- DESCRIPTION:
-- Explicitly drops all potential variations of fn_convertir_presupuesto_a_orden before recreating it.
-- This cleans up the duplicate function signatures caused by previous migrations.

-- Drop the function with the OLD signature (7 arguments)
...
-- Drop the function with the NEW signature (8 arguments) just to be safe
DROP FUNCTION IF EXISTS public.fn_convertir_presupuesto_a_orden(
    p_presupuesto_id uuid,
    p_fecha_entrega_estimada timestamp with time zone,
    p_notas_adicionales text,
...
-- Recreate the function with the correct signature (8 arguments)
