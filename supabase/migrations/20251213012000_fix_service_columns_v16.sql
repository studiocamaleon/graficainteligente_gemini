-- v16: Fix Service Table Columns
-- Goal: Fix "column does not exist" errors for ordenes_trabajo_servicios.
-- Corrections:
-- 1. Remove 'nombre' (does not exist). Map product_name to 'descripcion'.
-- 2. Rename 'precio_total' -> 'subtotal'.
-- 3. Remove 'estado' (does not exist).
...
BEGIN
  ...
  LOOP
    ...
    IF (v_item.configuracion->>'es_servicio_global')::boolean = true THEN
       -- FIX: Column mapping for ordenes_trabajo_servicios
       INSERT INTO ordenes_trabajo_servicios (
         orden_id,
         servicio_id,
         descripcion,     -- Was 'nombre' (error) + 'descripcion'
         precio_unitario,
         cantidad,
         subtotal        -- Was 'precio_total' (error)
         -- Remove 'estado' (error)
       ) VALUES (
         v_orden_id,
         (v_item.configuracion->>'servicio_id')::uuid,
         -- Combine Name + Desc for the single text column
         CASE 
