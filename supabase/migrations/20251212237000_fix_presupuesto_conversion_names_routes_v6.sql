/*
  # Fix v6: Flag de Cobro para Servicios Globales
  
  ## Problemas Resueltos
  1. **Rutas Vacías (UI)**: Aunque no se generaban rutas en BD, el frontend mostraba un contenedor de "Ruta Vacía".
     Esto ocurre porque el frontend de Ordenes espera el flag `es_servicio_cobro: true` en la configuración 
     para identificar items que son solo de facturación y ocultar la UI de producción.
  
  ## Cambios
  - Se inyecta `es_servicio_cobro: true` en el JSONB de `configuracion` al insertar en `ordenes_trabajo_items`
    cuando se detecta que es un servicio global.
*/
...
BEGIN
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      -- Determinar nombre correcto: Prioridad Producto Nombre
      ...
      -- Preparar Configuración: Agregar flag es_servicio_cobro si corresponde
      v_configuracion_final := COALESCE(v_item.configuracion, '{}'::jsonb);
      IF v_es_servicio_global THEN
         v_configuracion_final := v_configuracion_final || '{"es_servicio_cobro": true}'::jsonb;
      END IF;

      -- Insertar SIEMPRE en ordenes_trabajo_items
      INSERT INTO ordenes_trabajo_items (
        ...
        v_item.precio_unitario_final, 
        v_item.precio_total,
        v_configuracion_final
      ) RETURNING id INTO v_nuevo_item_id;

      -- Generar rutas: SOLO si NO es un servicio global
      IF NOT v_es_servicio_global THEN
      ...
      ELSE
        -- Log para debug
        RAISE NOTICE 'Skipping route generation for Global Service: % (flagged as servicio_cobro)', v_item_descripcion;
      END IF; -- Fin check NOT servicio global
