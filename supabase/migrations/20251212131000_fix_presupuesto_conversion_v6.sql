-- Migration to fix fn_convertir_presupuesto_a_orden (v6)
-- Fixes:
-- 1. Removes all columns that do not exist in ordenes_trabajo_items:
--    - company_id, orden, producto_nombre, categoria, unidad_medida, 
--    - subtotal, descuento, total (mapped to precio_total), precio_unitario (mapped to precio_unitario_final)
-- 2. Maps correct valid columns:
--    - tipo_item, descripcion (added in recent migrations)
...
BEGIN
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      -- Strictly map only existing columns in ordenes_trabajo_items
      -- ID, orden_id, producto_id, cantidad, configuracion, 
      -- precio_base, precio_servicios, precio_acabados, precio_unitario_final, precio_total
