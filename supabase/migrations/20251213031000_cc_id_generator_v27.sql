-- v27: Reuse Existing CC ID Generator
-- Goal: Use 'generate_numero_orden_copiado' to ensure consistent ID generation and avoid duplicates.
-- We recreate the generator first to be safe (idempotent).
...
BEGIN
  ...
  -- Count existing orders created today
...
BEGIN
  ...
  -- HELPER: Generate CC Number if needed (Centralized Function)
