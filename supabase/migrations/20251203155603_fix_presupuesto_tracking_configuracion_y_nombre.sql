/*
  # Fix Presupuesto Tracking - Configuración y Nombre Empresa

  1. Cambios
    - Agregar campo `configuracion` a items en tracking
    - Incluir `name` de la empresa además de `legal_name`
    - Mejorar información retornada para mejor visualización

  2. Objetivo
    - Mostrar detalles de configuración en vista pública de tracking
    - Usar nombre amigable de empresa en lugar de razón social
    - Permitir mejor experiencia de usuario en tracking público
*/
...
COMMENT ON FUNCTION fn_get_public_presupuesto_tracking(varchar) IS
  'Obtiene información pública de un presupuesto mediante su tracking token. Incluye configuración de productos y nombre de empresa.';

-- Grant acceso público a la función
