-- v23: Fix Origen mapping to CanalVenta
-- Goal: Update 'centro_copiado_ordenes.origen' to use the Budget's 'canal_venta'.
-- User Input: "El origen es el canal de venta".
-- Solution: Set origen = COALESCE(v_presupuesto.canal_venta, 'Mostrador').
...
DECLARE
  ...
  v_origen text; -- New variable to hold resolved origin
BEGIN
  -- Validar estado
  ...
  -- Resolve Origen/Canal Venta once
  ...
  INSERT INTO ordenes_trabajo (
    ...
    v_origen, -- Use resolved origin/canal
  ...
  IF v_es_centro_copiado THEN
     INSERT INTO centro_copiado_ordenes (
       ...
       total,
       observaciones,
       origen,  -- Set to same as canal_venta
       requiere_factura,
       canal_venta, -- Set to same as origen
       created_at,
       updated_at,
       created_by
     ) VALUES (
       ...
       v_origen, -- Fixed: Matches canal_venta
       COALESCE(p_requiere_factura, false),
       v_origen, -- Fixed: Matches origen
       NOW(),
       NOW(),
       auth.uid()
