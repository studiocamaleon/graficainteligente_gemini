/*
  # Agregar canal de venta 'App Mobile' y actualizar RPC

  ## Cambios
  1. Actualizar constraint check_canal_venta para incluir 'App Mobile'.
  2. Actualizar función RPC para aceptar parámetro de canal de venta.
*/

-- 1. Actualizar Constraint
ALTER TABLE ordenes_trabajo DROP CONSTRAINT IF EXISTS check_canal_venta;
ALTER TABLE ordenes_trabajo ADD CONSTRAINT check_canal_venta 
...
CREATE OR REPLACE FUNCTION fn_api_create_orden_copiado(
  ...
  p_canal_venta text DEFAULT 'Web' -- Nuevo parámetro con default
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO ordenes_trabajo (
    ...
    v_cliente_final_id,
    p_user_id,
    p_canal_venta, -- Usar el parámetro
    'borrador',
    v_numero_orden,
    p_user_id,
    p_user_id
