-- =====================================================
-- FIX: IVA Calculation in Budget to Order Conversion
-- Corrects the issue where subtotal, subtotal_iva and total were not correctly 
-- calculated when p_requiere_factura is true.
-- =====================================================
...
DECLARE
  ...
  -- Nuevas variables para cálculos de IVA
...
BEGIN
  ...
  -- 2. Calcular Totales según Facturación
  -- El total del presupuesto es el neto (subtotal - descuentos)
  IF COALESCE(p_requiere_factura, false) THEN
    v_subtotal_iva := ROUND(v_presupuesto.total * 0.21, 2);
    v_total_orden := v_presupuesto.total + v_subtotal_iva;
  ELSE
    v_subtotal_iva := 0;
    v_total_orden := v_presupuesto.total;
  END IF;

  -- 3. Validar Pago Inicial (contra el nuevo total)
  IF p_monto_pago IS NOT NULL THEN
    IF p_monto_pago <= 0 THEN
      RAISE EXCEPTION 'El monto del pago debe ser mayor a cero';
    END IF;
    IF p_monto_pago > v_total_orden THEN
      RAISE EXCEPTION 'El monto del pago no puede ser mayor al total de la orden (%)', v_total_orden;
    END IF;
  ...
  -- 4. Crear Orden de Trabajo (ÚNICA)
  INSERT INTO ordenes_trabajo (
    company_id, cliente_id, vendedor_id, canal_venta, numero_orden,
    fecha_estimada_entrega, notas_internas, subtotal, subtotal_iva, 
    total_descuentos, total, estado, requiere_factura, created_by
  )
  VALUES (
    ...
    v_presupuesto.subtotal, v_subtotal_iva, v_presupuesto.total_descuentos, 
    v_total_orden, 'pendiente', COALESCE(p_requiere_factura, false), auth.uid()
  )
  RETURNING id INTO v_orden_id;

  -- 5. Actualizar estado del Presupuesto
  UPDATE presupuestos
  ...
  FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id LOOP
    ...
    VALUES (
      ...
      COALESCE(v_item.configuracion, '{}'::jsonb), COALESCE(v_item.precio_base, 0),
      COALESCE(v_item.precio_servicios, 0), COALESCE(v_item.precio_acabados, 0),
      v_item.precio_unitario_final, v_item.precio_total, COALESCE(v_item.tiempo_produccion_dias, 0)
    )
    RETURNING id INTO v_nuevo_item_id;

    -- 7. Manejo de Rutas de Producción
    ...
    FOR v_ruta_db IN SELECT * FROM presupuestos_items_rutas WHERE presupuesto_item_id = v_item.id ORDER BY orden LOOP
      ...
      VALUES (
        v_nuevo_item_id, v_presupuesto.company_id, v_ruta_db.tipo_etapa, v_ruta_db.paso_id,
        v_ruta_db.paso_nombre, v_ruta_db.orden, v_ruta_db.es_modificado, v_ruta_db.comentario_vendedor
      );
    ...
    IF NOT v_has_db_routes AND p_rutas_personalizadas IS NOT NULL AND (p_rutas_personalizadas->v_item.id::text) IS NOT NULL THEN
        ...
        FOR v_ruta_json IN SELECT * FROM jsonb_array_elements(v_rutas_item) LOOP
          ...
          VALUES (
            v_nuevo_item_id, v_presupuesto.company_id, (v_ruta_json.value->>'etapa')::text,
            (v_ruta_json.value->>'paso_id')::uuid, (v_ruta_json.value->>'paso_nombre')::text,
            (v_ruta_json.value->>'orden')::integer, true, (v_ruta_json.value->>'comentario_vendedor')::text
          );
          v_has_db_routes := true;
        END LOOP;
    END IF;
