-- Fix finalized-order Wati trigger for multi-tenant/prod:
-- 1) remove hardcoded dev URL/secrets
-- 2) derive edge endpoint from DB settings
-- 3) send tenant company name as nombre_empresa
...
BEGIN
  ...
  v_config_url := nullif(current_setting('app.edge_function_url', true), '');
  ...
  ELSE
    RAISE WARNING '[Notify Trigger] Missing app.edge_function_url/app.supabase_url. Skipping WhatsApp notification.';
    RETURN NEW;
  END IF;
