-- ==============================================================================
-- FIX: Robust Route Generation V2
-- 1. Adds Fallback Heuristic: Check config for 'cantidad_copias' if category fails.
-- 2. Ensures dynamic finish injection works even if base route lookup was via fallback.
-- ==============================================================================
...
DECLARE
  ...
  v_is_copy_center boolean := false;
BEGIN
  ...
  -- A. Check Explicit Category
  IF (p_categoria ILIKE 'Centro de Copiado' OR p_categoria = 'centro_copiado') THEN
      v_is_copy_center := true;
  END IF;

  -- B. Fallback Heuristic: Check Config Content
  IF NOT v_is_copy_center AND (p_configuracion ? 'cantidad_copias' OR p_configuracion ? 'cantidad_hojas') THEN
      v_is_copy_center := true;
  END IF;

  -- C. Route Lookup
  IF v_is_copy_center THEN
  ...
  -- D. Standard Product Lookup (if still null)
  ...
  ---------------------------------------------------------------------------
  -- 2. GENERACIÓN DE PASOS ESTÁNDAR
  ---------------------------------------------------------------------------
  ...
  LOOP
    ...
    CASE v_paso_record.tipo_condicion
      ...
      WHEN 'servicio_con_nivel' THEN
        SELECT s INTO v_servicio_match FROM jsonb_array_elements(v_servicios) as s 
        ...
        IF v_servicio_match IS NOT NULL THEN
          ...
          ELSE
             BEGIN
               SELECT paso_id INTO v_paso_especifico_id FROM servicios_niveles_precio WHERE servicio_id = (v_servicio_match->>'servicio_id')::uuid AND nombre = v_nivel_aplicado LIMIT 1;
      ...
      WHEN 'acabado_con_nivel' THEN
        SELECT a INTO v_acabado_match FROM jsonb_array_elements(v_acabados) as a
        ...
        IF v_acabado_match IS NOT NULL THEN
          ...
          ELSE
             BEGIN
               SELECT paso_id INTO v_paso_especifico_id FROM acabados_niveles_precio WHERE acabado_id = (v_acabado_match->>'acabado_id')::uuid AND nombre = v_nivel_aplicado LIMIT 1;
             EXCEPTION WHEN OTHERS THEN v_paso_especifico_id := v_paso_record.paso_id; END;
          END IF;
           IF v_paso_especifico_id IS NOT NULL AND v_paso_especifico_id IS DISTINCT FROM v_paso_record.paso_id THEN
             SELECT nombre INTO v_paso_nombre FROM pasos WHERE id = v_paso_especifico_id;
      ...
      WHEN 'tecnologia_tinta' THEN
        IF v_item_tecnologia_id IS NOT NULL AND v_tinta_codigo IS NOT NULL THEN
             v_incluir_paso := true;
             BEGIN
               SELECT paso_id INTO v_paso_especifico_id FROM tecnologias_tintas_pasos WHERE tecnologia_id = v_item_tecnologia_id::uuid AND tinta = v_tinta_codigo LIMIT 1;
               IF v_paso_especifico_id IS NOT NULL AND v_paso_especifico_id IS DISTINCT FROM v_paso_record.paso_id THEN
                 SELECT nombre INTO v_paso_nombre FROM pasos WHERE id = v_paso_especifico_id;
               END IF;
             EXCEPTION WHEN OTHERS THEN v_paso_especifico_id := v_paso_record.paso_id; END;
    ...
    IF v_incluir_paso THEN
      INSERT INTO ordenes_trabajo_items_rutas (company_id, orden_item_id, tipo_etapa, paso_id, paso_nombre, orden, es_modificado, origen_plantilla_id) 
      VALUES (p_company_id, p_orden_item_id, v_paso_record.etapa, v_paso_especifico_id, v_paso_nombre, v_paso_record.orden, false, v_paso_record.id);
      v_count := v_count + 1;
    END IF;
  END LOOP;

  ---------------------------------------------------------------------------
  -- 3. INYECCIÓN DINÁMICA DE PASOS EXTRA
  ---------------------------------------------------------------------------
