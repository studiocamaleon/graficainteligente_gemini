...
CREATE OR REPLACE FUNCTION public.fn_convertir_presupuesto_a_orden(p_presupuesto_id uuid, p_fecha_entrega_estimada timestamp with time zone, p_notas_adicionales text DEFAULT NULL::text, p_monto_pago numeric DEFAULT NULL::numeric, p_medio_cobro_id uuid DEFAULT NULL::uuid, p_referencia_pago text DEFAULT NULL::text, p_rutas_personalizadas jsonb DEFAULT NULL::jsonb, p_requiere_factura boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_project_url text := 'https://velbpmbndvovczruzkzg.supabase.co';
  v_anon_key text := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZlbGJwbWJuZHZvdmN6cnV6a3pnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQ5NTM0MTgsImV4cCI6MjA4MDUyOTQxOH0.NaUzS0Ra1LOvWoVvj1is1c2PmdzcBT5elYDu5WcfSKw';
BEGIN
  -- 1. Validar estado
  ...
  -- 7. Notify using CORRECT URL and TOKEN from Env
  PERFORM net.http_post(
    url := v_project_url || '/functions/v1/enviar-notificacion-orden',
