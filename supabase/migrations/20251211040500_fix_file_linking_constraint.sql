/*
  # Fix File Linking Constraint & Prices
  
  Updates fn_api_create_orden_copiado to:
  1. Set orden_temporal_id = NULL when linking files (satisfies check constraint).
  2. Maintain price null safety (COALESCE).
*/
...
BEGIN
  ...
  -- 5. Vincular Archivos (FIX CONSTRAINT)
  IF p_archivos IS NOT NULL AND jsonb_array_length(p_archivos) > 0 THEN
    ...
    LOOP
      ...
      SET 
        ...
        orden_temporal_id = NULL -- FIX: Limpiar ID temporal para cumplir constraint XOR
