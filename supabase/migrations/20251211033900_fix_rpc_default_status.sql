/*
  # Corregir estado inicial en RPC para coincidir con check constraints

  ## Problema
  La función RPC usaba el estado 'borrador', pero la tabla `ordenes_trabajo` solo permite:
  ('pendiente', 'en_proceso', 'finalizada', 'entregada', 'cancelada').

  ## Solución
  Actualizar la función para usar estado 'pendiente' al crear la orden de trabajo.
*/
...
CREATE OR REPLACE FUNCTION fn_api_create_orden_copiado(
  ...
  p_canal_venta text DEFAULT 'Web'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 3. Insertar Orden de Trabajo (CORREGIDO ESTADO A 'pendiente')
  INSERT INTO ordenes_trabajo (
    ...
    p_user_id,
    p_canal_venta,
    'pendiente', -- ANTES: 'borrador' (Inválido) -> AHORA: 'pendiente' (Válido)
    v_numero_orden,
    p_user_id,
    p_user_id
