/*
  # Crear función para generar ruta de producción de un item

  1. Función Principal
    - `fn_generar_ruta_produccion_item`: Genera la ruta de producción para un item de orden
    - Obtiene el ruta_produccion_id del producto
    - Lee los pasos de rutas_produccion_pasos
    - Evalúa condiciones según configuración del item
    - Inserta pasos que correspondan en ordenes_trabajo_items_rutas

  2. Tipos de Condiciones
    - sin_condicion: Siempre se incluye
    - servicio_sin_nivel: Incluir si tiene el servicio
    - servicio_con_nivel: Incluir si tiene servicio con nivel específico
    - acabado_sin_nivel: Incluir si tiene el acabado
    - acabado_con_nivel: Incluir si tiene acabado con nivel específico
    - tecnologia_tinta: Incluir si usa tecnología + tinta específica

  3. Notas
    - Reemplaza la función obsoleta fn_copiar_ruta_desde_plantilla
    - Compatible con la nueva arquitectura de rutas_produccion
    - Maneja pasos condicionales y obligatorios
*/

-- Primero, eliminar la función antigua si existe
DROP FUNCTION IF EXISTS fn_copiar_ruta_desde_plantilla(uuid, uuid, uuid);
...
'Genera la ruta de producción para un item de orden basándose en la ruta asignada al producto y evaluando condiciones según la configuración del item';
