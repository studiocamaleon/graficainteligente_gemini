-- Migration v12: Fix Product Name Mapping in Conversion
-- DESCRIPTION:
-- 1. Ensures 'ordenes_trabajo_items' has 'producto_nombre' column (required for correct Order Detail display).
-- 2. Updates 'fn_convertir_presupuesto_a_orden' to explicitly map 'presupuestos_items.producto_nombre' to 'ordenes_trabajo_items.producto_nombre'.
-- 3. Decouples 'descripcion' so it only contains extra details (or NULL), preventing "Producto" from overriding the real name.

-- 1. Ensure Column Exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'ordenes_trabajo_items' AND column_name = 'producto_nombre') THEN
        ALTER TABLE ordenes_trabajo_items ADD COLUMN producto_nombre text;
    END IF;
END $$;

-- 2. Clean up previous function (reuse the nuclear options from v11 to be safe)
DO $$ 
DECLARE 
    r RECORD;
BEGIN 
...
-- 3. Recreate Function with Correct Mapping
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      INSERT INTO ordenes_trabajo_items (
        ...
        tipo_item,
        producto_id,
        producto_nombre, -- NEW COLUMN MAPPED
        descripcion, 
        cantidad,
        precio_base,
        precio_servicios,
        ...
        v_tipo_item_orden,
        v_item.producto_id,
        v_item.producto_nombre, -- INSERT REAL NAME
        v_item.descripcion, -- INSERT DESC AS IS (may be null) instead of coalescing deeply
        v_item.cantidad,
        v_item.precio_base,
