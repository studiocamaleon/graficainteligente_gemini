/*
  # Fix v2: Mover Servicios Globales de Presupuesto a Servicios Adicionales en Orden
  
  ## Problema
  Al convertir un presupuesto que tiene "Servicios Globales", estos podrían no detectarse correctamente
  debido a la falta de `producto_categoria` en algunos casos legacy o errores de mapeo previos.
  
  ## Solución
  Actualizar `fn_convertir_presupuesto_a_orden` para ser más robusto en la detección:
  1. Chequear flag `es_servicio_global`.
  2. Fallback: Chequear si el nombre empieza con `[Servicio]` (ignorando categoría si es null).
...
BEGIN
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      -- Detectar si es Servicio Global (Robust check)
      v_es_servicio_global := 
        -- Check 1: Flag explícito en configuración
        (COALESCE(v_item.configuracion->>'es_servicio_global', 'false') = 'true')
        OR 
        -- Check 2: Patrón de nombre legado (ignorando categoría si es null)
        (v_item.producto_nombre ILIKE '[Servicio]%');

      IF v_es_servicio_global THEN
         -- Insertar en SERVICIOS ADICIONALES
         -- Nota: Si viene de wizard, ya tiene un precio asignado
         INSERT INTO ordenes_trabajo_servicios (
            ...
            v_orden_id,
            NULLIF(v_item.configuracion->>'servicio_id', '')::uuid, 
         ...
         RAISE NOTICE 'Item % movido a ordenes_trabajo_servicios (Global)', v_item.producto_nombre;

      ELSE
        -- Insertar en ITEMS DE PRODUCCION
        ...
        IF v_item.tipo_item = 'producto_sistema' AND v_item.producto_id IS NOT NULL THEN
          BEGIN
            SELECT fn_generar_ruta_produccion_item(
              v_nuevo_item_id,
              v_item.producto_id,
              v_item.producto_categoria, -- Puede ser null si el bug frontend persistía, pero fn de rutas deberia manejarlo o fallar (pero manejamos excepcion)
              COALESCE(v_item.configuracion, '{}'::jsonb),
              v_presupuesto.company_id
