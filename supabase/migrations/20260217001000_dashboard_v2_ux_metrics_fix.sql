-- Dashboard v2.1 UX metrics fixes
-- 1) Add progreso_porcentaje in fn_dashboard_operativo_v2 for OT/CC deliveries.
-- 2) Emit NULL (instead of 0) in cumplimiento diario when no evaluadas that day.
    ...
    SELECT
      ...
      COALESCE(jsonb_agg(
        jsonb_build_object(
          'date', to_char(d.day, 'YYYY-MM-DD'),
          'label', to_char(d.day, 'DD/MM'),
          'value', CASE WHEN COALESCE(k.evaluadas, 0) = 0 THEN NULL
  ...
  IF v_scope = 'ot' THEN
    ...
    FROM (
      ...
      LEFT JOIN LATERAL (
        ...
        LEFT JOIN public.ordenes_trabajo_items_rutas r ON r.item_id = i.id
