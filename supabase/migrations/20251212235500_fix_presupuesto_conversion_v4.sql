/*
  # Fix v4: Alineación Definitiva de Conversión Presupuesto a OT (Estilo Wizard)

  ## Lógica Revisada
  1. Los items marcados como "Servicios Globales" (flag `es_servicio_global` o nombre `[Servicio]%`)
     SE INSERTAN en `ordenes_trabajo_items` para que aparezcan en el listado y facturación.
  2. PERO para estos ítems de servicio, NO se llama a `fn_generar_ruta_produccion_item`. 
     Esto evita que tengan ruta propia.
  3. Los ítems de producto (Targets), que ahora vienen con la configuración actualizada desde el frontend 
     (incluyendo `servicios_seleccionados`), generarán sus rutas incluyendo los pasos del servicio.

  ## Cambios
  - Revertir la inserción en `ordenes_trabajo_servicios`.
  - Conditional skip de generación de ruta para `es_servicio_global`.
*/
  ...
  ELSE
    ...
    FOR v_item IN SELECT * FROM presupuestos_items WHERE presupuesto_id = p_presupuesto_id AND tipo_item != 'centro_copiado' LOOP
      ...
      IF NOT v_es_servicio_global THEN
          ...
          END IF;
      ELSE
        RAISE NOTICE 'Item % es Servicio Global. No se genera ruta propia.', v_item.producto_nombre;
