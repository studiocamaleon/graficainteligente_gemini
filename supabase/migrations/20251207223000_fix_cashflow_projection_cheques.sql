-- =============================================
-- MIGRATION: 20251207223000_fix_cashflow_projection_cheques.sql
-- Description: Fixes fn_get_cashflow_projection to use cheques_cartera instead of cheques table.
-- =============================================
...
BEGIN
    ...
    movements AS (
        ...
        SELECT 
            fecha_estimada_entrega::date as fecha,
            (total - COALESCE((SELECT SUM(monto) FROM ordenes_trabajo_pagos WHERE orden_id = ot.id), 0)) as monto,
            'wip_in' as type
        FROM ordenes_trabajo ot
        WHERE company_id = p_company_id
          AND estado IN ('confirmado', 'en_produccion')
          AND fecha_estimada_entrega >= CURRENT_DATE
          -- Only include if client doesn't use current account to avoid double counting with liquidaciones
          AND NOT EXISTS (
    ...
    running_balance AS (
        SELECT 
            ...
            egreso_tarjetas,
            egreso_recurrentes,
            (ingreso_cheques + ingreso_liquidaciones + ingreso_wip) as total_ingresos,
            (egreso_cheques + egreso_tarjetas + egreso_recurrentes) as total_egresos,
            ((ingreso_cheques + ingreso_liquidaciones + ingreso_wip) - (egreso_cheques + egreso_tarjetas + egreso_recurrentes)) as saldo_diario,
            SUM((ingreso_cheques + ingreso_liquidaciones + ingreso_wip) - (egreso_cheques + egreso_tarjetas + egreso_recurrentes)) OVER (ORDER BY fecha) + v_saldo_inicial as saldo_acumulado
        FROM daily_agg
    )
