-- Dashboard operativo v2: priorizar órdenes con despacho en "Próximas Entregas" (scope OT)
  ...
  IF v_scope = 'ot' THEN
    SELECT COALESCE(
      jsonb_agg(
        row_to_json(t)
        ORDER BY
          CASE WHEN t.requiere_despacho THEN 0 ELSE 1 END,
          t.fecha_estimada_entrega ASC
      ),
      '[]'::jsonb
    )
    ...
    FROM (
      SELECT
        ...
        COALESCE(o.requiere_despacho, false) AS requiere_despacho,
        (timezone(p_tz, o.fecha_estimada_entrega)::date - timezone(p_tz, now())::date)::int AS dias_restantes,
        o.estado,
        CASE
      ...
      ORDER BY
        COALESCE(o.requiere_despacho, false) DESC,
        o.fecha_estimada_entrega ASC
  ...
  ELSE
    ...
    FROM (
      SELECT
        ...
        (timezone(p_tz, o.fecha_entrega_estimada)::date - timezone(p_tz, now())::date)::int AS dias_restantes,
        o.estado,
        CASE
