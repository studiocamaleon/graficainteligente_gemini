-- Migration: Update Debug Function to Show Step Conditions
-- Date: 2025-12-15
-- Description: Updates fn_debug_ruta_restore to return details of technology steps.
...
BEGIN
  ...
  -- Get keys as array safetly
  SELECT jsonb_agg(keys) INTO v_config_keys 
  FROM (SELECT jsonb_object_keys(v_configuracion) as keys) t;
  ...
  IF v_ruta_id IS NOT NULL THEN
    ...
    -- Get details of tech steps
    SELECT jsonb_agg(jsonb_build_object(
      'paso_id', paso_id,
      'configuracion_condicion', configuracion_condicion
    )) INTO v_step_details
    FROM rutas_produccion_pasos
    WHERE ruta_id = v_ruta_id 
    AND tipo_condicion = 'tecnologia_tinta';
  ...
  v_result := jsonb_build_object(
    ...
    'config_keys', COALESCE(v_config_keys, '[]'::jsonb),
    'tech_id_found', v_tech_id_in_config,
    'tech_nombre_found', v_tech_nombre_in_config,
    'step_conditions_details', COALESCE(v_step_details, '[]'::jsonb)
  );
