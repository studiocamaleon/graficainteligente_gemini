-- =====================================================
-- FIX: Robust Copy Center Route Logic
-- 1. Ensure 'Ruta Centro de Copiado' exists.
-- 2. Update conversion function to force category parameter.
-- =====================================================

-- 1. IDEMPOTENT ENSURE ROUTE
DO $$
DECLARE
  v_company_id uuid;
  v_estacion_id uuid;
...
BEGIN
  ...
  FOR v_company_id IN SELECT id FROM companies WHERE status = 'active' LOOP
      ...
      IF v_estacion_id IS NULL THEN
          ...
          VALUES (v_company_id, 'Centro de Copiado', 'Estación para trabajos de impresión rápida y anillados', true)
          RETURNING id INTO v_estacion_id;
      END IF;

      -- B. Step "Impresión / Copiado"
      ...
      IF v_paso_id IS NULL THEN
          ...
          VALUES (v_company_id, 'Impresión / Copiado', 'Produccion', v_estacion_id, true)
          RETURNING id INTO v_paso_id;
      END IF;

      -- C. Route "Ruta Centro de Copiado"
      ...
      IF v_ruta_id IS NULL THEN
          ...
          VALUES (v_company_id, 'Ruta Centro de Copiado', 'Flujo estándar para items de copiado rápido', true)
          RETURNING id INTO v_ruta_id;
      END IF;

      -- D. Link
      IF NOT EXISTS (SELECT 1 FROM rutas_produccion_pasos WHERE ruta_id = v_ruta_id AND paso_id = v_paso_id) THEN
          INSERT INTO rutas_produccion_pasos (ruta_id, etapa, paso_id, orden, es_obligatorio, tipo_condicion)
          VALUES (v_ruta_id, 'Produccion', v_paso_id, 1, true, 'sin_condicion');
      END IF;
